...
func TestChunkServer_IngestChunks_DocumentTopicIDError(t *testing.T) {
	db := failDBTX()
	s := NewChunkServer(store.NewChunkStore(db), store.NewDocumentStore(db), nil, &mockBackend{}, &mockAuthorizer{})
	ctx := systemCtx()
...
func TestChunkServer_IngestChunks_AuthorizerError(t *testing.T) {
	...
	s := NewChunkServer(store.NewChunkStore(db), store.NewDocumentStore(db), nil, &mockBackend{}, &mockAuthorizer{checkErr: errors.New("denied")})
	ctx := systemCtx()
...
func TestChunkServer_IngestChunks_BackendStoreError(t *testing.T) {
	...
	s := NewChunkServer(store.NewChunkStore(db), store.NewDocumentStore(db), nil, backend, &mockAuthorizer{})
	ctx := systemCtx()
...
func TestRetrievalServer_Search_QueryTextNoEmbedder(t *testing.T) {
	db := failDBTX()
	chunkStore := store.NewChunkStore(db)
...
func TestRetrievalServer_Search_QueryTextEmbedError(t *testing.T) {
	db := failDBTX()
	chunkStore := store.NewChunkStore(db)
	authz := &mockAuthorizer{}
	backend := &mockBackend{}
...
func TestRetrievalServer_Search_QueryTextNeitherProvided(t *testing.T) {
	db := failDBTX()
	chunkStore := store.NewChunkStore(db)
	authz := &mockAuthorizer{}
	backend := &mockBackend{}
...
func TestNilPrincipal_ChunkServer(t *testing.T) {
	...
	authz := &mockAuthorizer{}
	backend := &mockBackend{}
	s := NewChunkServer(cs, ds, nil, backend, authz)
	ctx := context.Background()
