...
func TestTopicStore_Integration_ConfigBinding(t *testing.T) {
	...
	topic, err := topicStore.Create(ctx, "bound-topic", "Bound Topic", "", "system:test", &llm.ID, &emb.ID, &prompt.ID, &vb.ID)
	if err != nil {
	...
	topic2, err := topicStore.Create(ctx, "unbound-topic", "Unbound", "", "system:test", nil, nil, nil, nil)
	if err != nil {
