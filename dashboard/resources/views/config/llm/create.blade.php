...
@section('title', 'Create LLM Config')

@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold">Create LLM Config</h2>
    </div>

    <div class="bg-white rounded shadow p-6 max-w-2xl">
        <form method="POST" action="{{ route('config.llm.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea id="parameters"
                          ...
                          class="w-full px-3 py-2 border border-slate-300 rounded text-sm text-slate-800 font-mono focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-slate-500">{{ old('parameters') }}</textarea>
                @error('parameters')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
