...
@section('title', 'API Key Configs')

@php use Carbon\Carbon; @endphp

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-semibold">API Key Configs</h2>
        <a href="{{ route('config.apikey.create') }}"
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="{{ route('config.apikey.edit', $config['id']) }}"
                                   class="text-slate-400 hover:text-blue-600 transition-colors" title="Edit">
                                    <x-heroicon-o-pencil-square class="w-5 h-5" />
                                </a>

                                @if (empty($config['is_default']))
                                    <form method="POST" action="{{ route('config.apikey.default', $config['id']) }}">
                                        @csrf
                                        <button type="submit"
                                ...
                                <form method="POST"
                                      action="{{ route('config.apikey.destroy', $config['id']) }}"
                                      x-data
                                      x-on:submit.prevent="if (confirm('Are you sure you want to delete this API key config?')) $el.submit()">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-slate-400">
                            No API key configs found. Create one to get started.
                        </td>
                    </tr>
                @endforelse
            </tbody>
