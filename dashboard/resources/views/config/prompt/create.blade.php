...
@section('title', 'Create Extraction Prompt')

@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold">Create Extraction Prompt</h2>
    </div>

    <div class="bg-white rounded shadow max-w-2xl">
        <form method="POST" action="{{ route('config.prompt.store') }}" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="prompt" id="prompt" rows="6" required
                          class="w-full rounded border-slate-300 shadow-sm text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('prompt') }}</textarea>
            </div>
