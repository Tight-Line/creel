...
func TestTopicStore_Integration_ConfigBinding(t *testing.T) {
	...
	topic, err := topicStore.Create(ctx, "bound-topic", "Bound Topic", "", "system:test", &llm.ID, &emb.ID, &prompt.ID)
	if err != nil {
	...
	topic2, err := topicStore.Create(ctx, "unbound-topic", "Unbound", "", "system:test", nil, nil, nil)
	if err != nil {
	...
	updated, err := topicStore.Update(ctx, topic2.ID, "Unbound Updated", "", &llm.ID, nil, nil)
	if err != nil {
